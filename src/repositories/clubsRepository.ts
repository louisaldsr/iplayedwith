...
type ClubRow = { id: string; name: string; sport: SportId; logo_url?: string | null }
...
export async function listBySport(db: SupabaseClient, sport: SportId, q?: string): Promise<Club[]> {
  let query = db.from('clubs').select('id, name, sport, logo_url').eq('sport', sport).order('name')
...
export async function insert(db: SupabaseClient, club: Club): Promise<Club> {
  const { error } = await db
    .from('clubs')
    .insert({ id: club.id, name: club.name, sport: club.sport, logo_url: club.logoUrl ?? null })
  if (error) throw new Error(error.message)
  return club
}
